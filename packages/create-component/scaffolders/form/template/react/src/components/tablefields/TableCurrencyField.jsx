...
export function TableCurrencyField({ field, value, onChange, onBlur, disabled }) {
    ...
    useEffect(
        function syncFromValue() {
            const [parsedAmount, parsedCurrency] = parsePayload(value, defaultCurrency)
            setAmount(parsedAmount)
            setSelectedCurrency(parsedCurrency)
            setDisplayValue(parsedAmount != null ? formatAmount(parsedAmount, decimalPoint) : '')
        },
    ...
    const isFixed = currencyTypes.length === 1 && currencyTypes[0] === selectedCurrency
    const hasDropdown = currencyTypes.length > 1 || (currencyTypes.length === 1 && !isFixed)

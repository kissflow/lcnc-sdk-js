...
export function TableMultiSelectField({ field, value, onChange, onBlur, disabled, getFieldOptions }) {
        ...
        <Popover open={open} onOpenChange={setOpen}>
                        ...
                        {displayLabels.length === 0 ? (
                            ...
                            <>
                                {displayLabels.slice(0, 2).map((label) => (
                                    <Badge key={label} variant="secondary" className="text-xs px-1.5 py-0 h-5 shrink-0">
                                        {label}
                                    </Badge>
                                ))}
            ...
            <PopoverContent className="w-56 p-1" align="start">
                <ScrollArea className="max-h-48">
                    {options.length === 0 ? (
                        <p className="text-sm text-gray-400 py-2 px-2">No options</p>
                    ) : (
                        options.map((opt) => (
                            <button
                                key={getId(opt)}
